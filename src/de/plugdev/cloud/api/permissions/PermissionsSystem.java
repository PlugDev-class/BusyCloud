package de.plugdev.cloud.api.permissions;
...
import de.plugdev.cloud.api.permissions.netlistener.DecodePermissionsListener;
...
public class PermissionsSystem {
	...
	public void init(String string) {
		ConsoleOutput.write(ConsoleOutput.CYAN, "[PERMISSIONS] §3Initiating PermissionsSystem...");
		...
		try {
			...
			for (String line : lines) {
				if (!line.startsWith("#")) {
					...
					groups.add(group);
					ConsoleOutput.write(ConsoleOutput.CYAN, "[PERMISSIONS] §3Registered group (" + group.getGroupId() + ", " + group.getGroupName() + ", " + group.getGroupHeight() + ")");
				}
			}
			lines.clear();
