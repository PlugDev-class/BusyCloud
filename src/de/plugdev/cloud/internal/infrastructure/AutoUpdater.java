...
//						ApplicationInterface.getAPI().getGuiInterface().pushStatusMessage("[CORE] No Cloudupdate found. You're up-to-date!");
	...
	/*
	 * This method autodownloads the spigotcloudbridge and the bungeecloudbridge.
	 * 
	 * @since 1.02
	 */
	@SneakyThrows
	public void doBridges() {
		...
		bungeeBridge = new URL("https://github.com/PlugDev-class/BusyCloud_BungeeCloudBridge/releases/download/1.0" + bungeeSubversion + "/BungeeCloudBridge.jar");
		while (continueState) {
		...
		FileUtils.download(
				"https://github.com/BusyCloud-Service/BusyCloud_SpigotCloudBridge/releases/download/fork-1.0/BusyCloud-Spigot.jar",
				"backend/downloads/SpigotCloudBridge.jar");
		FileUtils.download(bungeeBridge.getProtocol() + "://" + bungeeBridge.getHost() + bungeeBridge.getPath(),
				"backend/downloads/BungeeCloudBridge.jar");
	}
